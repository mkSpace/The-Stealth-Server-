...
DWORD CCircularQueue::GetSize()
{
	int max = mQueueHead > mQueueTail ? mQueueHead : mQueueTail;
	int min = mQueueHead < mQueueTail ? mQueueHead : mQueueTail;
...
BOOL CCircularQueue::Pop(VOID **object)
{
	CThreadSync Sync;
	if (!object)
		return FALSE;
