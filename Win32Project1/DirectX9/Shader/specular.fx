...
VS_OUTPUT vs_main( VS_INPUT Input )
{
	...
	float3 viewDir = normalize(Output.position.xyz - cameraPosition.xyz);
...
float4 ps_main( PS_INPUT Input ) : COLOR
{
	...
	float4 tex = tex2D( SpecularSampler, Input.tex);

	return tex * ambientColor * float4( specular + diffuse, 1 );
}
